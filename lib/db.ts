...
export function getFiltros(req:any, res:any){
...
export function getLimit(req:any, res:any){
...
export async function Post_Query(//pool:any, 
  select: string,filt:any, plimit:number = 50, req:any, res:any, next:any ) {

  const conn = await Connect(//pool
    ); 
  try {
    let where = ' where 1=1 ';     
    let limit = '';
...
export async function Simple_Query(//ppool:any, 
  sql: any, close:boolean = true) {
  degublog('------Simple_Query------begin-'); 
  const conn    = await <any>Connect(//ppool
    );
  try {    
    degublog(sql);     
    const result  = await Query(conn,sql, true);
...
export async function Simple_Insert_Query(//pool:any, 
sql: string) {
...
export async function Insert_Query(//pool:any, 
...
export async function setProperty(modulo:string,sql:string, json:any){    
  //  console.log('export async function setProperty....');   
   // let props     = await Simple_Query(sql); 
  //  console.log('props....1');
  //  console.log(props);
    //
   // let x = 0;
   ...
   //   prop = props[x];     
     // await setQueryJson(prop.sql, '_rows_'+modulo, json);
